...
struct Rendu
{
    ...
    {
        R_min = 1.0 + sqrt(1.0 - a2) + 1e-2; //Horizon du trou noir, erreurs numériques si pas le 1e-2 (avec RK4)
...
struct Disk
{
    ...
    void precalc()
    {
        R_min2 = pow((R_min - 0.5), 2); 
        R_max2 = pow((R_max + 0.5), 2) + 2.; //marge pour la detection avant de calculer le point de collision exact
...
struct Blackhole
{
    float a, a2;//Parametre de spin

    //calculer l'orbite la plus petite stable
    float inner_orbit(void)
    {
        float z1 = 1 + cbrt(1 - a2) * (cbrt(1 + a) + cbrt(1 - a)); //racine troisième
        float z2 = sqrt(3 * a2 + z1 * z1);
        return 3 + z2 - sqrt((3 - z1) * (3 + z1 + 2 * z2)); //z2 - .... (prograde) z2 + ... (retrograde)
    }
