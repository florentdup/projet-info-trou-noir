...
def start_window():
    ...
    label = Label(params, text='Choisissez les paramètres de la simulation :', font=(
        "Calibri Bold", 12))
    label.grid(row=0, column=0, stick=E)

    label_gauche = Label(
        params, anchor=W, text='Paramètres du trou noir :', font=("Calibri", 12))
    label_gauche.grid(row=1, column=0, sticky=W)

    label_droit = Label(
        params, anchor=E, text='Paramètres de la caméra :', font=("Calibri", 12))
    label_droit.grid(row=1, column=1, sticky=W)

    # Parametre de Kerr
    var_a = DoubleVar()
    a = Scale(params, from_=0.05, to=0.5, digits=2, variable=var_a,
              resolution=0.01, orient=HORIZONTAL, label="Parametre de Kerr", length=250)
    a.set(.5)
    a.grid(row=2, rowspan=2, column=0, sticky=W)

    # Rayon maximal disque d'accrétion
    RAdisk = Scale(params, from_=10, to=25, orient=HORIZONTAL,
                   label="Rayon maximal du disque d'accrétion", length=250, resolution=0.01)
    RAdisk.grid(row=4, column=0, sticky=W)
    RAdisk.set(16.)

    # position de la caméra
    camera_pos_x = Scale(params, from_=-25, to=25, orient=HORIZONTAL,
                         label="Coordonnée x de la caméra", length=250, resolution=0.01)
    camera_pos_x.grid(row=4, column=1, sticky=W)
    camera_pos_x.set(20)

    camera_pos_y = Scale(params, from_=-25, to=25, orient=HORIZONTAL,
                         label="Coordonnée y de la caméra", length=250, resolution=0.01)
    camera_pos_y.grid(row=5, column=1, sticky=W)
    camera_pos_y.set(0)

    camera_pos_z = Scale(params, from_=-25, to=25, orient=HORIZONTAL,
                         label="Coordonnée z de la caméra", length=250, resolution=0.01)
    ...
    R_inf = Scale(params, from_=max(15., RAdisk.get(), d)+1, to=40,
                  orient=HORIZONTAL, label="Rayon à l'infini", length=250, resolution=0.01)
    R_inf.grid(row=5, column=0, sticky=W)

    checkbuttonsurech = ttk.Checkbutton(params, text="Sur-échantillonage")
    checkbuttonsurech.grid(row=6, column=0, sticky=W, padx=10)
    ...
    def demarrer_programme(encours=False):
        if encours == False:
            encours=True
            surech = checkbuttonsurech.instate(['selected'])

            # Faire une copie de la string, puis la séparer
            tmp = variable.get()[:].split(',')
            ...
            liste_param.write(
                f"{a.get()} {RAdisk.get()} {camera_pos_x.get()} {camera_pos_y.get()} {camera_pos_z.get()} {R_inf.get()} {paramresolution}")
            liste_param.close()
            ...
            imageFinale = transform.resize(
                image, (width, height), anti_aliasing=True)

            plt.imsave("resultatFinal.png", imageFinale)

            imagefinaleFiltree = filtres(imageFinale, (width, height))
            plt.imsave("resultatFinalFiltré.png", imagefinaleFiltree)
            plt.figure()
            plt.imshow(imageFinale)
            plt.title("Résultat simulation")
            plt.figure()
            plt.imshow(imagefinaleFiltree)
    ...
    bouton_lancer = Button(params, text='Démarrer',
                           command=demarrer_programme, fg='red', activebackground='white')
    bouton_lancer.grid(row=8, columnspan=2, pady=10)

    params.mainloop()


bouton_commencer = Button(fenetre, text='Commencer', command=start_window,
                          bg='black', fg='orange', activebackground='white')
